...
protected:
    ...
    /* Get model's specify parameters from exposing.  */ 
    //virtual FrameShiftMs();
    //virtual FrameLenMs();
    //virtual GetNumMfccCoeffs();
...
/* Model's specify parameters are exposed by the following functions */

/**
 * @brief   Gets the FRAME_SHIFT_MS 
 * @return  a uint8_t
 **/
const uint8_t GetFrameShiftMs();

/**
 * @brief   Gets the FRAME_LEN_MS 
 * @return  a uint8_t
 **/
const uint8_t GetFrameLenMs();

/**
 * @brief   Gets the NUM_MFCC_COEFFS
 * @return  a uint8_t
 **/
