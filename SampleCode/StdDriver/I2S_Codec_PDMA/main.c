...
//static uint32_t s_au32PcmRxBuff[BUFF_NUM][BUFF_LEN] = {{0}};
//static DMA_DESC_T DMA_RXDESC[8];
...
void PDMA_Init(void)
{
  ...
  //  DMA_RXDESC[BUFF_NUM - 1].offset = (uint32_t)&DMA_RXDESC[BUFF_NUM] - (PDMA0->SCATBA);   /* Link to first description */
    ...
    //DMA_RXDESC[0].ctl = g_u32DMAConfig |PDMA_TBINTDIS_DISABLE;
    ...
    //
    //DMA_RXDESC[1].ctl = g_u32DMAConfig | PDMA_TBINTDIS_DISABLE;
    //DMA_RXDESC[1].src = (uint32_t)&I2S0->RXFIFO;
    //DMA_RXDESC[1].dest = (uint32_t)&s_au32PcmRxBuff[1];
    //DMA_RXDESC[1].offset = (uint32_t)&DMA_RXDESC[2] - (PDMA0->SCATBA);   /* Link to first description */
...
//    //PDMA_SetTransferMode(PDMA0, 1, PDMA_I2S0_RX, 1, (uint32_t)&DMA_RXDESC[4]);
